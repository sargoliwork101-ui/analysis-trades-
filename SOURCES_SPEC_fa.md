...
> `Http.kt` (تنها نقطه‌ی ورود شبکه: فقط http/https + سقف حجم) • `Fetcher.kt` (موتور fetch) • `SourceCatalog.kt` (منابع آماده) • `MarketKind.kt` (بازار هر منبع) • `JsonPath.kt` (مسیریابی JSON) • `Num.kt` (پاک‌سازی اعداد فارسی) • `TseModel.kt` (بورس تهران) • `QuoteRepo.kt` (کش و تاریخچه) • `Model.kt` (مدل داده)
...
- **همه‌ی خواندن‌های شبکه از یک ماژول** (`Http`) رد می‌شوند: یک `OkHttpClient` مشترک، فقط اسکیم `http`/`https` و سقف حجم پاسخ.
- **بازار هر منبع** با `MarketKind` مشخص می‌شود (نه با مقایسه‌ی رشته‌ای id) — همان چیزی که وضعیت «بورس/کریپتو/طلا و ارز/جهانی» و تصمیم‌های مخصوص هر بازار را می‌سازد.
...
| `unit` | string | (v1.14) واحد **مخصوص همین نماد** — خالی یعنی «واحد منبع». برای منبعی که هم انس دلاری دارد هم سکه‌ی تومانی، اجباری است |
| `scale` | double? | (v1.14) ضریب **مخصوص همین نماد** — `null` یعنی «ضریب منبع» |

در JSON منبع دلخواه، همان نحوِ کوتاهِ فیلد نماد پشتیبانی می‌شود: `code:label@unit`
(مثال: `ons:انس طلا (جهانی)@$`). آخرین `@` جداکننده‌ی واحد است تا خودِ واحد خراب نشود.
...
| `anomalyDetected` | bool | نمونه‌ی تازه جهش غیرعادی داشت و هنوز با نمونه‌ی دوم تأیید نشده است |
| `anomalyPct` | double? | درصد اختلاف نمونه‌ی مشکوک با آخرین قیمت سالم |
...
| کلاینت | **یکی برای کل برنامه** (v1.14) — استخر اتصال و نخ‌ها مشترک، بدون ساخت مکرر |
| سقف حجم پاسخ | JSON و HTML: **۸ مگابایت** (v1.14) — پاسخ بزرگ‌تر قبل از خوانده‌شدن کامل رد می‌شود |
| اسکیم مجاز | فقط `http://` و `https://` — هر اسکیم دیگری (`file:`، `content:`، …) رد می‌شود |
| redirect | حداکثر ۳ بار و فقط برای GET؛ downgrade از HTTPS به HTTP رد و credential در تغییر origin حذف می‌شود |
| credential روی HTTP | هدر حساس پیش از ارسال حذف و user-info یا query دارای API key/token/password/secret رد می‌شود |
| موازی‌سازی | حداکثر **۴ درخواست هم‌زمان** در `Fetcher` (باقی در صف) |

نکته: بعضی سایت‌های ایرانی هنوز روی HTTP سرو می‌شوند؛ cleartext فقط برای داده‌ی عمومی باقی مانده و هیچ هدر حساس روی آن ارسال نمی‌شود. پاسخ HTTP همچنان در مسیر شبکه قابل مشاهده و دست‌کاری است.
...
از نسخه‌ی ۱٫۱۵، endpoint شکست‌خورده (بر اساس scheme + host + path، بدون query حساس)
برای ۵ دقیقه به انتهای صف می‌رود؛ پس fallback بدون تکرار خطای primary استفاده می‌شود.
پس از cooldown، primary دوباره آزمایش می‌شود تا برای همیشه کنار گذاشته نشود. صفحه‌ی
«سلامت منابع» فقط میزبان، زمان پاسخ و وضعیت fallback را نگه می‌دارد و URL کامل یا هدرها را ثبت نمی‌کند.
...
- خطای HTTP: پیام = `HTTP {code} — {۶۰ کاراکتر اول بدنه‌ی پاک‌سازی‌شده}`؛ URL و credential پیش از نمایش/ذخیره حذف می‌شوند.
...
| unit | تومان |
| نمادها | `price_dollar_rl` دلار آمریکا (آزاد)، `price_eur` یورو، `price_gbp` پوند انگلیس، `geram18` طلای ۱۸ عیار، `geram24` طلای ۲۴ عیار، `mesghal` مثقال طلا، `sekee` سکه امامی، `sekeb` سکه بهار آزادی، `nim` نیم‌سکه، `rob` ربع‌سکه |
| نمادهای جهانی (v1.14) | `ons` انس جهانی طلا (واحد `$`، ضریب `1`)، `silver` انس جهانی نقره (`$`)، `tether_gold_xaut` طلای توکنیزه XAU (`$`) |

**مهم (v1.14):** ضریب و واحد در این منبع **per-symbol** است. سه نماد جهانیِ بالا واحد
`$` و ضریب `1.0` دارند؛ بقیه‌ی نمادها ضریب منبع `0.1` و واحد «تومان». اگر در اپ خود
تنها یک ضریب در سطح منبع بگذارید، انس طلا ۱۰ برابر کوچک‌تر و با واحد تومان نمایش داده
می‌شود — همان باگی که در v1.14 رفع شد.
...
### ۷.۵) بازارهای جهانی — TradingView (v1.14)

| مورد | مقدار |
|---|---|
| kind | `JSON_REST` (خواندن هم‌زمان نمادهای یک منبع، ولی هر نماد یک درخواست) |
| آدرس | `https://scanner.tradingview.com/symbol?symbol={symbol}&fields=close,change,change_abs,volume,description&no_404=true` |
| pricePath | `close` |
| changePath / حالت | `change` / `PERCENT` |
| volumePath | `volume` |
| نمادها | `OANDA:XAUUSD` طلا — انس جهانی، `TVC:SILVER` نقره، `TVC:USOIL` نفت WTI، `TVC:UKOIL` نفت برنت، `TVC:DXY` شاخص دلار، `BITSTAMP:BTCUSD` بیت‌کوین، `BITSTAMP:ETHUSD` اتریوم |
| واحد | `$` (شاخص دلار: `امتیاز`) |
| بازار | `MarketKind.GLOBAL` — باز از یکشنبه ۱۸:۰۰ نیویورک تا جمعه ۱۷:۰۰ (با احتساب ساعت تابستانی) |

نکات:

- اندپوینت **تک‌نمادِ** `scanner.tradingview.com/symbol` جواب می‌دهد؛ نسخه‌ی گروهی
  (`/symbols?symbols=A,B`) خطای ۴۰۴ می‌دهد — پس `batchTemplate` نگذارید.
- یک درخواست، کل داده‌ی نماد را می‌دهد (قیمت، درصد، تغییر مطلق، حجم و توضیح) —
  برای همین نیازی به اندپوینت دیگری نیست.
- در شبکه‌ی ایران ممکن است فیلتر باشد؛ منبع آماده‌ی TGJU (بخش ۷.۴) جایگزین بومیِ
  همان انس طلا است.
- هر نماد دلخواه دیگر (مثلاً `TVC:GOLD` یا `NASDAQ:AAPL`) را کاربر با همین آدرسِ
  الگو می‌تواند به‌عنوان منبع دلخواه اضافه کند.
...
*ساخته‌شده برای پروژه‌ی «نبض بازار» — نسخه‌ی سند: ۱٫۱۵ (هم‌گام با نسخه‌ی ۱٫۱۵ اپ)*
